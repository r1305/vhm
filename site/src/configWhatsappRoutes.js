...
router.put('/', soloSuperAdmin, async (req, res) => {
  try {
    ...
    if (existing.length > 0) {
      await pool.execute(
        'UPDATE config_whatsapp SET numero = ?, mensaje = ?, activo = ?, fecha_actualizacion = NOW() WHERE id = 1',
        [numero || null, mensaje || null, activo ? 1 : 0]
      );
    } else {
      await pool.execute(
        'INSERT INTO config_whatsapp (id, numero, mensaje, activo, fecha_actualizacion) VALUES (1, ?, ?, ?, NOW())',
        [numero || null, mensaje || null, activo ? 1 : 0]
      );
    }
