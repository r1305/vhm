...
const {
  getAllCatalog, getAccesosForUser, setAccesosForUser, listUsersWithAccesos,
} = require('./lib/accesos');

const router = Router();

router.use(async (req, res, next) => {
  try { await ensureSchema(); next(); } catch { res.status(503).json({ error: 'Servicio inicializándose' }); }
});
...
router.get('/mi-menu', async (req, res) => {
  try {
    ...
    res.json({ items });
  } catch (err) { console.error(err); res.status(500).json({ error: 'Error al obtener menú' }); }
});

router.get('/', requireSuperAdmin, async (req, res) => {
  try {
    const [catalog, users] = await Promise.all([getAllCatalog(), listUsersWithAccesos()]);
    res.json({ catalog, users });
  } catch (err) { console.error(err); res.status(500).json({ error: 'Error al obtener accesos' }); }
});
...
router.put('/usuario/:id', requireSuperAdmin, async (req, res) => {
  try {
    ...
    const [[target]] = await pool.execute(
      'SELECT id, rol, es_protegido FROM tribu_admins WHERE id = ?', [userId]
    );
  ...
  } catch (err) { console.error(err); res.status(400).json({ error: err.message || 'Error al guardar accesos' }); }
});
