  ...
  try {
    ...
    if (!tarjetaId && wantsAutoRenew) {
      ...
      if (vaultResult.ok && vaultResult.vault?.cardId) {
        ...
        try { await upsertSavedCard(req.tribuUser.id, vaultInfo); } catch (saveErr) { console.error('[tribu-pagos] guardar tarjeta BD', saveErr.message); }
      } else {
    ...
    if (!vaultInfo?.cardId) {
      ...
      if (fromCharge?.cardId) {
        ...
        if (wantsAutoRenew) {
          try { await upsertSavedCard(req.tribuUser.id, fromCharge); } catch (saveErr) { console.error('[tribu-pagos] guardar tarjeta BD (charge)', saveErr.message); }
        }
      }
    }
    ...
    if (outcome.status === 'approved') {
      try {
        await savePayerProfile(req.tribuUser.id, checkout.identificationType, checkout.identificationNumber, cfg.modo === 'produccion' ? email : null);
        tribuSuscripcionId = await activateNewSubscription({
          userId: req.tribuUser.id, planId: suscripcion_id, chargeId: outcome.charge_id,
        ...
        console.error('[tribu-pagos] suscripcion', dbErr.message);
    ...
    await recordCulqiTransaction(charge, { source: tarjetaId ? 'api_tarjeta_guardada' : 'api', tribuUserId: req.tribuUser.id, planId: suscripcion_id, tribuSuscripcionId, payerEmail: email });
...
router.post('/webhook', async (req, res) => {
  try {
    ...
    const cfg = await getCulqiConfig();
    const isChargeSuccess = String(eventType).includes('charge.succeeded') || String(eventType).includes('charge.success') || chargeData?.outcome?.type === 'venta_exitosa';
    if (isChargeSuccess) {
      const charge = chargeData?.object === 'charge' ? chargeData : await fetchCulqiCharge(cfg.secret_key, chargeId);
      const applied = await applyApprovedCharge(charge);
      await recordCulqiTransaction(charge, { source: 'webhook', tribuSuscripcionId: applied?.tribuSuscripcionId ?? null, payerEmail: charge.email || null });
    }
    res.sendStatus(200);
