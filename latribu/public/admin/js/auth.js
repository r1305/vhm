(function (global) {
  ...
  function isSuperAdmin() { return state.user && state.user.rol === 'SUPER_ADMIN'; }
  function isAdmin() { const r = state.user && state.user.rol; return r === 'SUPER_ADMIN' || r === 'ADMIN'; }
  ...
  function hasAccess(pageId) {
    ...
    return getMenuItems().includes(pageId);
  }
  ...
  async function ensureMenuItems() {
    ...
    try {
      const res = await fetch(AdminApi.API_BASE + '/accesos/mi-menu', { credentials: 'same-origin', headers: AdminApi.authHeaders() });
      if (!res.ok) return getMenuItems();
      const data = await res.json();
      const items = data.items || [];
      const u = state.user;
      if (u) { u.menuItems = items; localStorage.setItem('user', JSON.stringify(u)); }
      return items;
    } catch (_) { return getMenuItems(); }
  }
  ...
  function loadTheme() { applyTheme(state.theme === 'dark'); }
  ...
  function requireAuth() {
    if (!state.token) { global.location.href = 'login.html'; return false; }
    return true;
  }
