...
(function () {
  ...
  ready(function () {
    const { api, toast, esc, badge, fullName,
    ...
    async function loadAgenda() {
      try {
        ...
        if (dia) {
          ...
          qs.set('mes', `${agendaFechaActual.getFullYear()}-${String(agendaFechaActual.getMonth()+1).padStart(2,'0')}`);
        }
    ...
    document.getElementById('agendaMes')?.addEventListener('change', e => {
      ...
      const hoy = new Date();
      agendaFechaActual = (anio === hoy.getFullYear() && mes - 1 === hoy.getMonth())
        ? hoy : new Date(anio, mes - 1, 15);
