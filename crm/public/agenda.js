...
(function () {
      ...
      <div class="ag-cita-card" data-id="${c.id}">
        ...
        <div class="ag-cita-right">
          ...
          <div class="ag-cita-actions">
            <button class="btn-icon" data-cita-editar="${c.id}" title="Editar cita"><i class="fas fa-pen"></i></button>
  ...
  function bindCitaActions(container, citas) {
    window.CRM_CITAS.bindCitaActions(container, citas, refreshAgenda);
  }
  ...
  async function loadDetail(pacienteId) {
    ...
    try {
      ...
      detail.innerHTML = `
        ...
        ${timelineHTML}`;
      bindCitaActions(detail, citas);
      document.getElementById('agBackBtn')?.addEventListener('click', () => selectPaciente(null));
      document.getElementById('agBtnNuevaCita')?.addEventListener('click', () => {
        window.CRM_CITAS.showNuevaCita({
          paciente: p,
  ...
  document.getElementById('btnNuevaCita')?.addEventListener('click', () => {
    window.CRM_CITAS.showNuevaCita({
      ...
      onSuccess: (body) => {
        if (body?.paciente_id) pacienteSeleccionado = body.paciente_id;
        refreshAgenda();
      },
    });
  });
