...
(function () {
  ...
  function barH(container, rows, { labelKey, valueKey, color='#7c3aed', fmt=v=>v }={}) {
    ...
    container.innerHTML = rows.map(r => {
      const val = parseFloat(r[valueKey])||0, pct = Math.max(4,(val/max)*100);
  ...
  function donut(container, rows, { labelKey, valueKey }={}) {
    ...
    container.innerHTML = rows.map((r,i) => {
      const val = parseFloat(r[valueKey])||0, pct = total>0?Math.round((val/total)*100):0;
      return `<div style="display:flex;align-items:center;gap:8px;margin-bottom:7px">
  ...
  async function loadAnalitica() {
    ...
    try {
      ...
      document.getElementById('anaKpis').innerHTML = `
        ...
        <div class="kpi-card success"><div class="kpi-label">Conversiones</div><div class="kpi-value">${k.sesiones_convertidas||0}</div><div class="kpi-sub">Tasa: ${k.tasa_conversion||0}%</div></div>
        <div class="kpi-card" style="border-left:3px solid var(--info)"><div class="kpi-label">Tiempo promedio</div><div class="kpi-value" style="font-size:1.3rem">${fmtSeg(k.duracion_promedio)}</div><div class="kpi-sub">Scroll: ${k.scroll_promedio||0}%</div></div>
        <div class="kpi-card warning"><div class="kpi-label">Tasa de rebote</div><div class="kpi-value">${k.tasa_rebote||0}%</div><div class="kpi-sub">${k.rebotes||0} sesiones &lt;10s</div></div>`;

      sparkDual(document.getElementById('anaSesionesDia'), d.sesionesPorDia||[], { dateKey:'dia', v1Key:'sesiones', v2Key:'unicos' });
      barH(document.getElementById('anaFuentes'), d.porFuente||[], { labelKey:'fuente', valueKey:'sesiones', color:'#7c3aed' });
      ...
      if ((d.topPaginas||[]).length) {
        tpEl.innerHTML = `<table style="width:100%;font-size:12px;border-collapse:collapse">
          <thead><tr style="color:var(--text-muted)"><th style="text-align:left;padding:4px 6px">Página</th><th style="text-align:right;padding:4px 6px">Visitas</th><th style="text-align:right;padding:4px 6px">Únicos</th><th style="text-align:right;padding:4px 6px">Tiempo</th><th style="text-align:right;padding:4px 6px">Scroll</th></tr></thead><tbody>` +
      ...
      } else { tpEl.innerHTML = '<div class="list-empty">Sin datos</div>'; }

      donut(document.getElementById('anaDispositivos'), d.porDispositivo||[], { labelKey:'dispositivo', valueKey:'total' });
      donut(document.getElementById('anaNavegadores'), d.porNavegador||[], { labelKey:'navegador', valueKey:'total' });
      barH(document.getElementById('anaTopClicks'), d.topClicks||[], { labelKey:'elemento', valueKey:'clicks', color:'#d97706' });

      const erEl = document.getElementById('anaEventosRecientes');
      if ((d.eventosRecientes||[]).length) {
        const TIPO_BADGE = { conversion:'badge-green', form_submit:'badge-blue', click:'badge-yellow' };
      ...
      } else { erEl.innerHTML = '<div class="list-empty">Sin eventos recientes</div>'; }
