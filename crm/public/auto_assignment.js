      ...
      document.getElementById('tablaReglas').innerHTML = data.length
        ? data.map(r => `
          <tr>
            ...
            <td>${r.prioridad}</td>
            <td><button class="btn-icon danger" data-del="${r.id}" title="Eliminar"><i class="fas fa-trash"></i></button></td>
          </tr>`).join('')
  ...
  async function showNuevaRegla() {
    ...
    openModal('Nueva regla de asignación', `
      <div class="form-group">
        ...
        <input class="form-control" id="f_keyword" placeholder="ej: ansiedad, pareja, infantil…">
        <div style="font-size:11px;color:var(--text-muted);margin-top:4px">Si el motivo de consulta contiene esta palabra, el lead se asigna al terapeuta seleccionado.</div>
      </div>
      ...
      <div class="form-group"><label class="form-label">Prioridad (1 = más alta)</label><input type="number" min="1" max="99" class="form-control" id="f_prioridad" value="1"></div>`,
      async () => {
        const body = {
