...
(function () {
        ...
        ? data.map(h => `
          <div class="note-card">
            <div class="note-meta">
              ...
              <button class="btn-icon danger" data-del="${h.id}" style="padding:0 4px"><i class="fas fa-trash"></i></button>
            </div>
  ...
  function showNuevaNota() {
    openModal('Nueva nota clínica', `
      <div class="form-row">
        <div class="form-group"><label class="form-label">Fecha *</label><input type="date" class="form-control" id="f_fecha" value="${new Date().toISOString().slice(0,10)}"></div>
        <div class="form-group"><label class="form-label">Tipo</label>
          <select class="form-select" id="f_tipo">
            <option value="evolucion">Evolución</option><option value="evaluacion">Evaluación</option>
            <option value="derivacion">Derivación</option><option value="alta">Alta</option><option value="otro">Otro</option>
          </select>
        </div>
      </div>
      <div class="form-group"><label class="form-label">Nota clínica *</label><textarea class="form-control" id="f_nota" rows="6" placeholder="Descripción de la sesión…"></textarea></div>`,
      async () => {
  ...
  document.getElementById('historialPacienteSelect').addEventListener('change', e => {
    ...
    document.getElementById('btnNuevaNota').disabled = !pacienteId;
    if (pacienteId) loadHistorial();
